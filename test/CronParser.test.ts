...
describe("CronParser", () => {
  ...
  describe("parse", () => {
    ...
    test("should support quoted command parsing", () => {
      const result = parser.parse("*/5 * * * * echo \"Hello World\"");
    ...
    test("should parse lists with multiple values", () => {
      const result = parser.parse("0,15,30,45 1,12,23 5,10,15,20 2,4,6,8,10,12 1,3,5 /cmd");
    ...
    test("should parse complex mixed expressions", () => {
      const result = parser.parse("0,15,30,45 1-5 1,15,30 1-6/2 1-5 /usr/bin/backup");
    ...
    test("should handle command with spaces", () => {
      const result = parser.parse("0 0 * * * /usr/bin/command with spaces");

      expect(result.command).toBe("/usr/bin/command with spaces");
    });
    ...
    test("should throw for unknown field name error", () => {
      // Test parseField with unknown field
      expect(() => parser.parse("0 0 1 1 1 /cmd")).not.toThrow();
    });
  });
  ...
  describe("formatAndPrint", () => {
    ...
    test("should print formatted output with minimal fields", () => {
      const spy = jest.spyOn(console, "log").mockImplementation(() => {});
    ...
    test("should handle string values in output", () => {
      const spy = jest.spyOn(console, "log").mockImplementation(() => {});
